...
//=============================================================================
 /*:
 * @plugindesc v1.03 Have more control over the flow of the battle system
...
/*:ja
 * @plugindesc 戦闘システムをカスタムし、様々な要素を変更することができます
 * @author Yanfly Engine Plugins
 *
 * @param ---一般---
 * @default
 ...
 * プレイヤーに与えることができます。
 * このプラグインを用いると、スキルに 振り のアニメーションを付与することができ、
 * スキルごとに設定することも、全体に一括設定することも可能です。
 *
 * スキルノートタグ:
 *   <Cast Animation: x>
...
BattleManager.startAction = function () {
    ...
    //-------------------------------------
    console.log("action.makeTargets()");
...
BattleManager.checkBeforeCommon = function (action) {
    if (action && !this._execBeforeCommon && DataManager.isBeforeCommon(action.item())) {
        ...
        $gameVariables._data[11] = this._subject.index();
        console.log(this._subject);
        $gameVariables._data[11] = this._subject._name;
        if (this._subject.isEnemy()) { $gameVariables._data[11] += 1000 }
        this._phase = 'turn';
        return true;
    }
    return false;
